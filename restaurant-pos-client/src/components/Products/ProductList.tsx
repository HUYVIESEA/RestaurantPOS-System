    ...
    <div className="p-4 md:p-6 bg-slate-50 dark:bg-slate-900 min-h-screen text-slate-800 dark:text-slate-200">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4">
        <div>
          ...
          <div className="flex gap-3 mt-3 flex-wrap">
             ...
             <div className="px-3 py-1.5 rounded-xl text-sm font-medium border bg-blue-50 text-blue-800 border-blue-200 dark:bg-blue-900/30 dark:text-blue-500 dark:border-blue-900/50 flex gap-2 items-center">
                <span>Sắp hết</span>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4 bg-white dark:bg-slate-800 p-3 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
         <div className="relative w-full sm:w-96 flex items-center">
            ...
            <input 
              type="text" 
              className="w-full pl-10 pr-10 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-600/50 transition-all dark:text-white"
         ...
         <div className="flex gap-1 bg-slate-100 dark:bg-slate-900 p-1 rounded-xl border border-slate-200 dark:border-slate-700">
            <button
               className={`p-2 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-white dark:bg-slate-800 shadow-sm text-blue-600 dark:text-blue-500' : 'text-slate-500 hover:text-slate-900 dark:hover:text-slate-100'}`}
            ...
            <button
               className={`p-2 rounded-lg transition-all ${viewMode === 'list' ? 'bg-white dark:bg-slate-800 shadow-sm text-blue-600 dark:text-blue-500' : 'text-slate-500 hover:text-slate-900 dark:hover:text-slate-100'}`}
      ...
      <div className="overflow-x-auto pb-2 mb-6 scrollbar-hide">
         <div className="flex gap-2 min-w-max">
            <button
               className={`whitespace-nowrap px-4 py-2 rounded-xl border font-medium text-sm transition-all flex items-center gap-2 ${selectedCategory === null ? 'bg-blue-50 text-blue-800 border-blue-200 dark:bg-blue-900/30 dark:text-blue-500 dark:border-blue-900' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 hover:border-blue-300 dark:hover:border-blue-800 text-slate-700 dark:text-slate-300'}`}
               onClick={() => setSelectedCategory(null)}
            >
               <i className="fas fa-utensils"></i>
               Tất cả
               <span className={`px-2 py-0.5 rounded-full text-xs ${selectedCategory === null ? 'bg-blue-100 dark:bg-blue-900' : 'bg-slate-100 dark:bg-slate-700'}`}>{allProducts.length}</span>
            </button>
            {categories.map(category => {
               ...
               return (
                 <button
                    key={category.id}
                    className={`whitespace-nowrap px-4 py-2 rounded-xl border font-medium text-sm transition-all flex items-center gap-2 ${isActive ? 'bg-blue-50 text-blue-800 border-blue-200 dark:bg-blue-900/30 dark:text-blue-500 dark:border-blue-900' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 hover:border-blue-300 dark:hover:border-blue-800 text-slate-700 dark:text-slate-300'}`}
                    onClick={() => setSelectedCategory(category.id)}
                 >
                    <i className="fas fa-tag"></i>
                    {category.name}
                    <span className={`px-2 py-0.5 rounded-full text-xs ${isActive ? 'bg-blue-100 dark:bg-blue-900' : 'bg-slate-100 dark:bg-slate-700'}`}>
                       {count}
                    </span>
                 </button>
               );
                ...
                <div className="absolute top-3 left-3 flex flex-col gap-1">
                   {(product.stockQuantity !== undefined) && (
                     product.stockQuantity < 0 ? (
                        <span className="bg-blue-600 text-white px-2.5 py-1 rounded-lg text-xs font-bold shadow-sm flex items-center gap-1"><i className="fas fa-infinity"></i></span>
                     ) : product.stockQuantity === 0 ? (
                        <span className="bg-red-500 text-white px-2.5 py-1 rounded-lg text-xs font-bold shadow-sm">Hết hàng</span>
                     ) : product.stockQuantity < 10 ? (
                        <span className="bg-blue-600 text-white px-2.5 py-1 rounded-lg text-xs font-bold shadow-sm">Còn {product.stockQuantity}</span>
                     ) : null
                   )}
                </div>
              </div>
              ...
              <div className="p-5 flex flex-col flex-grow">
                  <div className="flex-grow">
                     <div className="flex justify-between items-start gap-2 mb-1">
                        ...
                        <span className="font-semibold text-blue-700 dark:text-blue-500 flex-shrink-0 text-lg">{formatCurrency(product.price)}</span>
                     </div>
                  ...
                  <div className="mt-auto">
                      ...
                      <div className="flex justify-end gap-2 pt-3 border-t border-slate-100 dark:border-slate-700">
                         {permissions.products.canEdit && (
                           <button
                             className="p-2.5 rounded-xl text-slate-500 hover:text-blue-700 hover:bg-blue-50 dark:hover:bg-slate-800 dark:hover:text-blue-500 transition-colors"
                             onClick={() => navigate(`/products/edit/${product.id}`)}
                             title="Chỉnh sửa"
                           >
