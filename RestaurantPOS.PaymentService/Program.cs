...
builder.ConfigureTracing("payment-service");
...
var port = builder.Configuration.GetValue<int>("PORT", 5003);
...
app.MapHealthChecks("/health/ready", new HealthCheckOptions { ResponseWriter = async (ctx, report) => { ctx.Response.ContentType = "application/json"; await ctx.Response.WriteAsync(JsonSerializer.Serialize(new { status = "ready", service = "payment-service" })); } });
